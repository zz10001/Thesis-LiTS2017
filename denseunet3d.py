...
def transition_block3d(x, stage, nb_filter, compression=1.0, dropout_rate=None):
    ...
    x = BatchNormalization(epsilon=eps, axis=4, name=conv_name_base+'_batch_normalization')(x)
...
def DenseUNet(img_input, nb_dense_block=4, growth_rate=48, nb_filter=96, reduction=0.0, dropout_rate=0.0):
    ...
    x = Conv2D(nb_filter, (7, 7), strides=(2, 2), name='convolution_1', use_bias=False, trainable=False)(x)
    ...
    x = Scale(axis=concat_axis, name='convolution_'+str(final_stage)+'_blk_scale', trainable=False)(x)
    x = Activation('relu', name='convolution_ReLU_'+str(final_stage)+'_blk')(x)
    box.append(x)

    up0 = UpSampling2D(size=(2, 2))(x)
    conv_up0 = Conv2D(768, (3, 3), padding="same", name="upsampling_0", trainable=False)(up0)
    bn_up0 = BatchNormalization(name="upsampling_0_batch_normalization", momentum=1, trainable=False)(conv_up0, training=False)
    ac_up0 = Activation('relu', name='upsampling_0_ReLU')(bn_up0)

    up1 = UpSampling2D(size=(2, 2))(ac_up0)
    conv_up1 = Conv2D(384, (3, 3), padding="same", name="upsampling_1", trainable=False)(up1)
    bn_up1 = BatchNormalization(name="upsampling_1_batch_normalization", momentum=1, trainable=False)(conv_up1, training=False)
    ac_up1 = Activation('relu', name='upsampling_1_ReLU')(bn_up1)

    up2 = UpSampling2D(size=(2, 2))(ac_up1)
    conv_up2 = Conv2D(96, (3, 3), padding="same", name="upsampling_2", trainable=False)(up2)
    bn_up2 = BatchNormalization(name="upsampling_2_batch_normalization", momentum=1, trainable=False)(conv_up2, training=False)
    ac_up2 = Activation('relu', name='upsampling_2_ReLU')(bn_up2)

    up3 = UpSampling2D(size=(2, 2))(ac_up2)
    conv_up3 = Conv2D(96, (3, 3), padding="same", name="upsampling_3", trainable=False)(up3)
    bn_up3 = BatchNormalization(name="upsampling_3_batch_normalization", momentum=1, trainable=False)(conv_up3, training=False)
    ac_up3 = Activation('relu', name='upsampling_3_ReLU')(bn_up3)

    up4 = UpSampling2D(size=(2, 2))(ac_up3)
    conv_up4 = Conv2D(64, (3, 3), padding="same", name="upsampling_4", trainable=False)(up4)
    bn_up4 = BatchNormalization(name="upsampling_4_batch_normalization", momentum=1, trainable=False)(conv_up4, training=False)
    ac_up4 = Activation('relu', name='upsampling_4_ReLU')(bn_up4)

    x = Conv2D(3, (1, 1), padding="same", name='2D-DenseUNet_classifier', trainable=False)(ac_up4)
...
def denseunet_3d(args):
    ...
    final_conv = Conv3D(64, (3, 3, 3), padding="same", name='final_convolution')(final)
    final_conv = Dropout(rate=0.1)(final_conv)
    final_bn = BatchNormalization(name="final_batch_normalization")(final_conv)
    final_ac = Activation('relu', name='final_ReLU')(final_bn)
    classifier = Conv3D(3, (1, 1, 1), padding="same", name='2d_3d_classifier')(final_ac)

    model = Model(inputs=img_input, outputs=classifier, name='auto3d_residual_conv')
